      ...
      <ion-content class="ion-padding">
        ...
        <div class=" mx-4 mt-4 flex justify-center ">
          <div class="relative inline-block w-10 mr-2 align-middle select-none transition duration-200 ease-in">
            <input @change="changeMode" type="checkbox" name="toggle" id="toggle" v-model="modeCliente" class="toggle-checkbox border-red-400 absolute block w-6 h-6 rounded-full bg-white border-4 appearance-none cursor-pointer" />
            <label for="toggle" class="toggle-label block overflow-hidden h-6 rounded-full bg-red-400 cursor-pointer border-red-400 "></label>
          </div>
          <label for="toggle" class="toggle-label text-xs text-gray-700 self-center ">{{ modeCliente.toString() == 'true' ? 'Cliente' : 'Especialista' }}</label>
        </div>
        <div class="w-full mt-4" v-if="route.path === '/tabs/tab1'">
          <ion-button @click="() => router.push('/register-especialista')" class="w-full font-bold bg-black rounded-md " color="black" >
            Registrarse
          </ion-button>
        </div>
        <div class="w-fit absolute bottom-0 m-4 right-0  ">
          <ion-button @click.prevent="logout" class="w-full font-bold " :color="`danger`">
            ...
            <svg fill="#fff" version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="20px" height="20px" viewBox="0 0 471.851 471.851" xml:space="preserve">
              <g>
                <g>
